...
def get_network_from_plans(arch_class_name, arch_kwargs, arch_kwargs_req_import, input_channels, output_channels,
                           allow_init=True, deep_supervision: Union[bool, None] = None, num_classes: int = None):
    ...
    # If num_classes is specified, use the classification variants from nnunetv2
    if num_classes is not None:
        if "ResidualEncoderUNet" in network_class:
            network_class = "nnunetv2.architectures.unet_with_classification.ResidualEncoderUNetWithClassification"
        elif "PlainConvUNet" in network_class:
            network_class = "nnunetv2.architectures.unet_with_classification.PlainConvUNetWithClassification"
