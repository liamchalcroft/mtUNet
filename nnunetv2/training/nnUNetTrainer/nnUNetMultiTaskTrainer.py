...
class nnUNetMultiTaskTrainer(object):
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            self.network = self.build_network_architecture(
                ...
                3 # number of classes - hardcoded for now, need to integrate into plans ideally
            ).to(self.device)
    ...
    def build_network_architecture(architecture_class_name: str,
        ...
        If classification is enabled (num_classes is not None), will return a network with both segmentation 
    ...
    def build_loss(self):
        ...
        self.classification_loss = self._build_classification_loss()
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            # Forward pass returns (seg_outputs, class_output)
            seg_outputs, class_output = self.network(data)
